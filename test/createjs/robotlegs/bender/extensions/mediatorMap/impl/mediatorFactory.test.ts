...
describe("MediatorFactory", () => {
    ...
    function mediatorsCreatedWithGuards(...guards: any[]): number {
        ...
        const mediators: any[] = factory.createMediators(new createjs.Container(), createjs.Container, [mapping]);
    ...
    it("mediator_is_created", () => {
        ...
        const mediator: IMediator = factory.createMediators(new createjs.Container(), createjs.Container, [mapping])[0];
        assert.instanceOf(mediator, CallbackMediator);
    });

    it("mediator_is_injected_into", () => {
        const expected: number = 128;
        const mapping: IMediatorMapping = new MediatorMapping(createTypeFilter([createjs.Container]), InjectedMediator);
        injector.bind(Number).toConstantValue(expected);
        const mediator: InjectedMediator = factory.createMediators(new createjs.Container(), createjs.Container, [mapping])[0];
    ...
    it("mediatedItem_is_injected_as_exact_type_into_mediator", () => {
        ...
        const mapping: IMediatorMapping = new MediatorMapping(createTypeFilter([createjs.Container]), ViewInjectedMediator);
        const mediator: ViewInjectedMediator = factory.createMediators(expected, createjs.Container, [mapping])[0];
        assert.equal(mediator.mediatedItem, expected);
    });

    it("mediatedItem_is_injected_as_requested_type_into_mediator", () => {
        const expected: createjs.Container = new createjs.Container();
        const mapping: MediatorMapping = new MediatorMapping(createTypeFilter([createjs.DisplayObject]), ViewInjectedAsRequestedMediator);
        const mediator: ViewInjectedAsRequestedMediator = factory.createMediators(expected, createjs.Container, [mapping])[0];
        assert.equal(mediator.mediatedItem, expected);
    });
    ...
    it("hook_receives_mediator_and_mediatedItem", () => {
        ...
        const mapping: MediatorMapping = new MediatorMapping(createTypeFilter([createjs.Container]), ViewInjectedMediator);
    ...
    it("same_mediators_are_returned_for_mappings_and_mediatedItem", () => {
        const mediatedItem: createjs.Container = new createjs.Container();
        const mapping1: MediatorMapping = new MediatorMapping(createTypeFilter([createjs.Container]), ViewInjectedMediator);
        const mapping2: MediatorMapping = new MediatorMapping(createTypeFilter([createjs.DisplayObject]), ViewInjectedAsRequestedMediator);
        const mediators1: any[] = factory.createMediators(mediatedItem, createjs.Container, [mapping1, mapping2]);
        const mediators2: any[] = factory.createMediators(mediatedItem, createjs.Container, [mapping1, mapping2]);
    ...
    it("expected_number_of_mediators_are_returned_for_mappings_and_mediatedItem", () => {
        ...
        const mapping1: MediatorMapping = new MediatorMapping(createTypeFilter([createjs.Container]), ViewInjectedMediator);
        const mapping2: MediatorMapping = new MediatorMapping(createTypeFilter([createjs.DisplayObject]), ViewInjectedAsRequestedMediator);
        const mediators: any = factory.createMediators(mediatedItem, createjs.Container, [mapping1, mapping2]);
    ...
    it("removeAllMediators_removes_all_mediators_from_manager", () => {
        ...
        const mapping1: IMediatorMapping = new MediatorMapping(createTypeFilter([createjs.Container]), CallbackMediator);
        const mapping2: IMediatorMapping = new MediatorMapping(createTypeFilter([createjs.DisplayObject]), ViewInjectedAsRequestedMediator);
        manager = new MediatorManager(factory, injector.get(IDisplayObjectObserverFactory));
        let managerMock = sinon.mock(manager);
